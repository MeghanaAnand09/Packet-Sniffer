...
def printPacketsV4(filter, data, raw_data):
    ...
    if proto == 1 and (len(filter) == 0 or filter[1] == 1):
        ...
        print ("*******************ICMP***********************")
        print ("\tICMP type: %s" % (icmp_type))      #type8 ICMP req type 0 ICMP reply
        print ("\tICMP code: %s" % (code))           #what kind of ICMP message it is : 0 destn network unreachable 1: destn host unreachable
    ...
    # TCP
        ...
        print('protocol: {}\nSource address: {}\nDestination address: {}'.format(proto, src, target))
